...
void PairKolmogorovCrespiFull::KC_neigh()
{
  ...
  // store KC neighs of owned and ghost atoms for calculating normals
  // only the 3 closest same-layer neighbors are used (graphene bonding)
  // if more than 3 candidates exist, keep the 3 nearest by distance

  const int MAX_KC_CAND = 256;
  int j_cand[MAX_KC_CAND];
  double rsq_cand[MAX_KC_CAND];
  ...
  for (ii = 0; ii < allnum; ii++) {
    ...
    // collect all same-molecule neighbors within cutoff
    for (jj = 0; jj < jnum && n < MAX_KC_CAND; jj++) {
      j = jlist[jj];
      j &= NEIGHMASK;
      jtype = map[type[j]];
      delx = xtmp - x[j][0];
    ...
    // keep only the 3 closest; partial selection sort
    int nkeep = (n < 3) ? n : 3;
    for (int k = 0; k < nkeep; k++) {
      int imin = k;
      for (int m = k + 1; m < n; m++) {
        if (rsq_cand[m] < rsq_cand[imin]) imin = m;
      }
      if (imin != k) {
        int tj = j_cand[k];
        double trsq = rsq_cand[k];
        j_cand[k] = j_cand[imin];
        rsq_cand[k] = rsq_cand[imin];
        j_cand[imin] = tj;
        rsq_cand[imin] = trsq;
      }
      neighptr[k] = j_cand[k];
