...
def get_MCMC_inputs(model_name, calc_type="python", supercells=1,
    ...
    if model_name.startswith("MK"):
        ...
        if os.path.exists(f"{_BEST_FIT_PARAMS_SUBDIR}/MK_best_fit_params.npz"):
            ...
            p0 = data["params"]
            bounds["hopping"] = data["bounds"]
            params["hopping"], ypred_bestfit["hopping"] = fit_model(mk_hopping,xdata_train["hopping"],ydata_train["hopping"],p0,yshift=0,zero_shift_data=False,bounds=bounds["hopping"])
            np.savez(
        ...
        from blg_model_builder_v2.tb_models import get_acsf_hoppings_sk
        ...
        if os.path.exists(f"{_BEST_FIT_PARAMS_SUBDIR}/interlayer_LETB_best_fit_params.npz"):
            ...
            p0 = data["params"]
            bounds["hopping"] = data["bounds"]
            params["hopping"], ypred_bestfit["hopping"] = fit_model(letb_interlayer,xdata_train["hopping"],ydata_train["hopping"],p0,yshift=0,zero_shift_data=False,bounds=bounds["hopping"])
            np.savez(
        ...
        if hyperparameters["nn_val"] ==1:
            calc["hopping"] = letb_intralayer_t01
            model_name = "intralayer_LETB_NN_val_1"
        elif hyperparameters["nn_val"] ==2:
            calc["hopping"] = letb_intralayer_t02
            model_name = "intralayer_LETB_NN_val_2"
        elif hyperparameters["nn_val"] ==3:
            calc["hopping"] = letb_intralayer_t03
            model_name = "intralayer_LETB_NN_val_3"
        else:
            raise ValueError(f"Unknown nn_val '{hyperparameters['nn_val']}' for LETB intralayer models")
