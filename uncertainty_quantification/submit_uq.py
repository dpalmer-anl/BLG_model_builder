...
def submit_batch_file_aurora(executable,batch_options,
                                 ...
                                 dir='cd /lus/flare/projects/qmchamm/dpalmer3/BLG_model_builder/PYMC_uncertainty_quanitification'):
...
if __name__=="__main__":
    ...
    batch_options_aurora = {
            "-A": "qmchamm",
            "-N": "job_name",
            "-l": "select=1:ncpus=1:ngpus=1,walltime=01:00:00,filesystems=flare",
            "-q": "prod",
            "modules":['frameworks']}
    ...
    if relaxation:
        uq_arr = ["mcmc"] #,"cv"]
        twist_angle = np.array([0.88,0.99,1.08,1.2,1.47,1.89,2.88])
    ...
    if band_structure:
        uq_arr = ["cv","mcmc"]
        twist_angle = np.array([0.88,0.99,1.08,1.2,1.47,1.89,2.88])
        model_tuple = [("Classical","None"),("TETB","MK")] 
        for mt in model_tuple:
            for u in uq_arr:
                if u=="cv":
                    npartitions=2
