...
if __name__=="__main__":
    ...
    if gpu_avail:
        k_node = np.asnumpy(k_node)
        k_dist = np.asnumpy(k_dist)
    ...
    efermi = (evals[nbands//2,0] + evals[(nbands-1)//2,0])/2
    fermi_ind = (nbands)//2
    ...
    ax.set_xlim(k_node[0], k_node[-1])
    fig.savefig("figures/"+TB_model+"_theta_"+str(theta)+"_graphene_GPU.png", bbox_inches='tight')
    plt.clf()
